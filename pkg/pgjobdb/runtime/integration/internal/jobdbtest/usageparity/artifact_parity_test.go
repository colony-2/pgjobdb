...
import (
	...
	jobdbtest "github.com/colony-2/pgjobdb/pkg/pgjobdb/runtime/integration/internal/jobdbtest"
)
...
type artifactRoundTripObservation struct {
	Chapter   jobdb.Chapter        `json:"chapter"`
	Runtime   normalizedArtifact   `json:"runtime"`
	Engine    normalizedArtifact   `json:"engine"`
	StoredRef jobdb.StoredArtifact `json:"storedRef"`
}
...
func startManualStorageJob(
	t *testing.T,
	ctx context.Context,
	submit func(context.Context, jobdb.SubmitJob) (jobdb.JobKey, error),
	runtime jobdb.WorkflowRuntime,
	tenantID string,
	jobID string,
) (jobdb.JobKey, jobdb.ExecutionLease) {
	t.Helper()

	jobKey, err := submit(ctx, jobdb.SubmitJob{
		TenantId: tenantID,
...
func TestRuntimeArtifactRoundTripParityAcrossBuiltInRuntimes(t *testing.T) {
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		...
		t.Run(harness.Name, func(t *testing.T) {
			...
			jobKey, lease := startManualStorageJob(t, ctx, func(ctx context.Context, start jobdb.SubmitJob) (jobdb.JobKey, error) {
			...
			chapterReq := jobdb.PutChapterRequest{
			...
			runtimeArtifact := mustReadRuntimeArtifactBytes(t, ctx, built.Runtime, jobdb.ArtifactRef{
				JobKey:  jobKey,
				Ordinal: 1,
				Name:    stored[0].Name,
				Digest:  stored[0].Digest,
			})
			engineArtifact := mustReadEngineArtifactBytes(t, ctx, built.Engine, jobKey.TenantId, jobdb.ArtifactKey{
				JobId:       jobKey.JobId,
				TaskOrdinal: 1,
				Name:        stored[0].Name,
	...
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		...
		t.Run(harness.Name, func(t *testing.T) {
			compareAcrossModes(t, harness, nil, func(t *testing.T, ctx context.Context, subject scenarioSubject) storedChapterObservation {
				...
				runtimeArtifact := mustReadRuntimeArtifactBytes(t, ctx, subject.Runtime(), jobdb.ArtifactRef{
					JobKey:  jobKey,
					Ordinal: 1,
					Name:    stored[0].Name,
					Digest:  stored[0].Digest,
				})
				engineArtifact := mustReadEngineArtifactBytes(t, ctx, subject.Engine(), jobKey.TenantId, jobdb.ArtifactKey{
					JobId:       jobKey.JobId,
					TaskOrdinal: 1,
					Name:        stored[0].Name,
					SizeBytes:   stored[0].Size,
...
func TestArtifactStorageOnTaskErrorParityAcrossBuiltInRuntimes(t *testing.T) {
	...
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		harness := harness
		t.Run(harness.Name, func(t *testing.T) {
			compareAcrossModes(t, harness, []workflow.WorkSet{ws}, func(t *testing.T, ctx context.Context, subject scenarioSubject) artifactErrorObservation {
				...
				chapter, err := subject.Runtime().GetChapter(ctx, jobdb.ChapterRef{JobKey: jobKey, Ordinal: 1})
				if err != nil {
	...
	for _, harness := range jobdbtest.BuiltInRuntimeHarnesses() {
		...
		t.Run(harness.Name, func(t *testing.T) {
			run := func(mode parityMode) artifactCleanupObservation {
				...
				return observeViaMode(t, harness, mode, []workflow.WorkSet{ws}, func(t *testing.T, ctx context.Context, subject scenarioSubject) artifactCleanupObservation {
					jobKey, err := subject.SubmitJob(ctx, jobdb.SubmitJob{
						TenantId: subject.built.WorkerTenantID,
						JobType:  job.Name(),
